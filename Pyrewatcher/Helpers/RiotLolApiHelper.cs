...
namespace Pyrewatcher.Helpers
{
  public class RiotLolApiHelper
  {
    ...
    public async Task<List<LolMatchDto>> MatchGetByMatchesList(List<LolMatch> matchesList)
    {
      var tasks = new List<Task<HttpResponseMessage>>();

      foreach (var match in matchesList)
      {
        tasks.Add(ApiClient.GetAsync($"https://{match.ServerApiCode}.api.riotgames.com/lol/match/v4/matches/{match.MatchId}"));
      }

      var responses = await Task.WhenAll(tasks);

      var output = new List<LolMatchDto>();
      ...
      for (var i = 0; i < matchesList.Count; i++)
      {
        ...
        if (responses[i].IsSuccessStatusCode)
        {
          var responseContent = await responses[i].Content.ReadAsAsync<LolMatchDto>();

          output.Add(responseContent ?? new LolMatchDto {GameId = matchesList[i].MatchId});
        }
        else
        {
          output.Add(new LolMatchDto {GameId = matchesList[i].MatchId});
        }
      }
    ...
    public async Task<List<MatchlistDto>> MatchGetMatchlistsByRiotAccountsList(List<RiotAccount> accountsList)
    {
      var tasks = new List<Task<HttpResponseMessage>>();
      ...
      {
        var serverApiCode = _utilities.GetServerApiCode(account.ServerCode);
        tasks.Add(ApiClient.GetAsync(
                    $"https://{serverApiCode}.api.riotgames.com/lol/match/v4/matchlists/by-account/{account.AccountId}?beginTime={beginTime}"));
      }

      var responses = await Task.WhenAll(tasks);

      var output = new List<MatchlistDto>();
      ...
      foreach (var response in responses)
      {
        ...
        if (response.IsSuccessStatusCode)
        {
          var responseContent = await response.Content.ReadAsAsync<MatchlistDto>();

          output.Add(responseContent ?? new MatchlistDto {SummonerId = accountsList[i].SummonerId});
        }
        else
        {
          output.Add(new MatchlistDto {SummonerId = accountsList[i].SummonerId});
        }
